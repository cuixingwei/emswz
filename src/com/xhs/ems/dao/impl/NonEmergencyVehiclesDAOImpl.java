...
public class NonEmergencyVehiclesDAOImpl implements NonEmergencyVehiclesDAO {
	...
	public Grid getData(Parameter parameter) {
		String sql = "select distinct pc.任务编码,pc.任务序号,pc.里程 into #pc from AuSp120.tb_PatientCase pc "
				...
				+ "where e.事件性质编码=1 and e.受理时刻 between :startTime and :endTime and e.事件类型编码=3  "
				+ "select 'ids' ids, COUNT(*) safeguard_times,sum(pc.里程) safeguard_distance into #temp2	from AuSp120.tb_TaskV t "
				+ "left outer join AuSp120.tb_EventV e on e.事件编码=t.事件编码	left outer join #pc pc on pc.任务序号=t.任务序号 and pc.任务编码=t.任务编码 	"
				+ "where e.事件性质编码=1 and e.受理时刻 between :startTime and :endTime and e.事件类型编码=5 "
				+ "select 'ids' ids,COUNT(*) clinic_times,sum(pc.里程) clinic_distance into #temp3	from AuSp120.tb_TaskV t "
				+ "left outer join AuSp120.tb_EventV e on e.事件编码=t.事件编码	left outer join #pc pc on pc.任务序号=t.任务序号 and pc.任务编码=t.任务编码 	"
				+ "where e.事件性质编码=1 and e.受理时刻 between :startTime and :endTime and e.事件类型编码=7 "
				+ "select 'ids' ids,COUNT(*) practice_times,sum(pc.里程) practice_distance into #temp4	from AuSp120.tb_TaskV t "
				+ "left outer join AuSp120.tb_EventV e on e.事件编码=t.事件编码	left outer join #pc pc on pc.任务序号=t.任务序号 and pc.任务编码=t.任务编码 	"
				+ "where e.事件性质编码=1 and e.受理时刻 between :startTime and :endTime and e.事件类型编码 in (8,9) "
				+ "select 'ids' ids,COUNT(*) inHospital_times,sum(pc.里程) inHospital_distance into #temp5 	from AuSp120.tb_TaskV t "
				+ "left outer join AuSp120.tb_EventV e on e.事件编码=t.事件编码	left outer join #pc pc on pc.任务序号=t.任务序号 and pc.任务编码=t.任务编码 	"
				+ "where e.事件性质编码=1 and e.受理时刻 between :startTime and :endTime and e.事件类型编码=3 and t.分站编码='001' "
				+ "select 'ids' ids,COUNT(*) xh_times,sum(pc.里程) xh_distance into #temp6 	from AuSp120.tb_TaskV t "
				+ "left outer join AuSp120.tb_EventV e on e.事件编码=t.事件编码	left outer join #pc pc on pc.任务序号=t.任务序号 and pc.任务编码=t.任务编码 	"
				+ "where e.事件性质编码=1 and e.受理时刻 between :startTime and :endTime and e.事件类型编码=6 "
				+ "select 'ids' ids,COUNT(*) other_times,sum(pc.里程) other_distance into #temp7 	from AuSp120.tb_TaskV t "
				+ "left outer join AuSp120.tb_EventV e on e.事件编码=t.事件编码	left outer join #pc pc on pc.任务序号=t.任务序号 and pc.任务编码=t.任务编码 	"
				+ "where e.事件性质编码=1 and e.受理时刻 between :startTime and :endTime and e.事件类型编码=10 "
