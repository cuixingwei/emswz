...
public class MissMealDAOImpl implements MissMealDAO {
	...
	public Grid getData(Parameter parameter) {
		String sql = "select distinct 任务序号,任务编码,随车医生,随车护士,司机 into #pc from AuSp120.tb_PatientCase  "
				...
				+ "SUM(case when (DATEPART(Hour,t.出车时刻)<12 and DATEPART(Hour,DATEADD(minute,15,t.到达医院时刻))>12) or (DATEPART(Hour,t.出车时刻)<18 and DATEPART(Hour,DATEADD(minute,15,t.到达医院时刻))>18) then 1 else 0 end)   times	"
				...
				+ "select pc.随车护士 name,COUNT(*) rate,	SUM(case when (DATEPART(Hour,t.出车时刻)<12 and DATEPART(Hour,DATEADD(minute,15,t.到达医院时刻))>12) or (DATEPART(Hour,t.出车时刻)<18 and DATEPART(Hour,DATEADD(minute,15,t.到达医院时刻))>18) then 1 else 0 end)   times	"
				+ "from #pc pc left outer join AuSp120.tb_TaskV t on t.任务编码=pc.任务编码 and t.任务序号=pc.任务序号	"
				+ "left outer join AuSp120.tb_AcceptDescriptV a on a.事件编码=t.事件编码 and a.受理序号=t.受理序号	"
