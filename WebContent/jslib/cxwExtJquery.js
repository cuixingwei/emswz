var cxw = cxw || {};
cxw.data = cxw.data || {};// 用于存放临时的数据或者对象
 ...
 * 增加命名空间功能
 * 
 * 使用方法：cxw.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
cxw.ns = function() {
	var o = {}, d;
 ...
 * 将form表单元素的值序列化成对象
 * 
 * @example cxw.serializeObject($('#formId'))
 * 
 * @author 崔兴伟
 * 
 * @requires jQuery
 ...
 * @returns object
 */
cxw.serializeObject = function(form) {
	var o = {};
 ...
 * @author 崔兴伟
 * 
 * @example cxw.formatString('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * 
 * @returns 格式化后的字符串
 */
cxw.formatString = function(str) {
 ...
 * @returns list
 */
cxw.stringToList = function(value) {
 ...
 * @author Klaus Hartl/klaus.hartl@stilbuero.de
 */
cxw.cookie = function(key, value, options) {
